...
- **If you build from source, do not keep the debug and release builds installed at once.** Only
  release APKs are published, so nobody downloading Trawl can hit this -- it is a bench hazard for
  anyone working on the code. The debug build carries a `.debug` package suffix, so the two are
  separate apps sharing `Download/Trawl`, and MediaStore records the owner per path: the second app
  gets `Postprocessing: Error opening input files: Permission denied` on any file the first one
  already downloaded. It looks like a broken build and is not one. Keep one installed, or give them
